...
struct params {
  ...
  int idThread; // cet entier correspond au num�ro de traitement associ� � un
                // thread
...
void *traitement(void *p) {
  ...
  for (int i = 1; i <= vPartage->nbZones; i++) {
    ...
    // dans cette partie, le traitement de la zone i est � faire en faisant
    // une simulation d'un long calcul (appel a calcul()
...
int main(int argc, char *argv[]) {
  ...
  srand(atoi(argv[1])); // initialisation de rand pour la simulation de
                        // longs calculs

  // cr�ation des threards
  for (int i = 0; i < atoi(argv[1]); i++) {
    tabParams[i].idThread = tabParams[i].vPartage =
